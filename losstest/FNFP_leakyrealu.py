...
def main():
    ...
    conv1 = Conv2D(neuron_default, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(input_layer)
    conv1 = Conv2D(neuron_default, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(conv1)
    pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)
    drop1 = Dropout(0.25)(pool1)

    conv2 = Conv2D(neuron_default * 2, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(drop1)
    conv2 = Conv2D(neuron_default * 2, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(conv2)
    pool2 = MaxPooling2D(pool_size=(2, 2))(conv2)
    drop2 = Dropout(0.25)(pool2)

    conv3 = Conv2D(neuron_default * 4, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(drop2)
    conv3 = Conv2D(neuron_default * 4, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(conv3)
    pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)
    drop3 = Dropout(0.25)(pool3)

    conv4 = Conv2D(neuron_default * 8, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(drop3)
    conv4 = Conv2D(neuron_default * 8, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(conv4)
    pool4 = MaxPooling2D(pool_size=(2, 2))(conv4)
    drop4 = Dropout(0.25)(pool4)

    ## Center point
    conv5 = Conv2D(neuron_default * 16, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(drop4)
    conv5 = Conv2D(neuron_default * 16, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(conv5)
    drop5 = Dropout(0.5)(conv5)

    up6 = UpSampling2D(size = (2,2))(drop5)
    up6 = Conv2DTranspose(neuron_default * 8, 2, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(up6)
    merge6 = concatenate([conv4,up6])
    drop6 = Dropout(0.25)(merge6)
    conv6 = Conv2D(neuron_default * 8, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(drop6)
    conv6 = Conv2D(neuron_default * 8, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(conv6)

    up7 = Conv2DTranspose(neuron_default * 4, 2, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(UpSampling2D(size = (2,2))(conv6))
    merge7 = concatenate([conv3,up7])
    drop7 = Dropout(0.25)(merge7)
    conv7 = Conv2D(neuron_default * 4, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(drop7)
    conv7 = Conv2D(neuron_default * 4, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(conv7)

    up8 = Conv2DTranspose(neuron_default * 2, 2, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(UpSampling2D(size = (2,2))(conv7))
    merge8 = concatenate([conv2,up8])
    drop8 = Dropout(0.25)(merge8)
    conv8 = Conv2D(neuron_default * 2, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(drop8)
    conv8 = Conv2D(neuron_default * 2, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(conv8)

    up9 = Conv2DTranspose(neuron_default, 2, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(UpSampling2D(size = (2,2))(conv8))
    merge9 = concatenate([conv1,up9])
    drop9 = Dropout(0.25)(merge9)
    conv9 = Conv2D(neuron_default, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(drop9)
    conv9 = Conv2D(neuron_default, 3, activation = tf.keras.layers.LeakyReLU(alpha=relu_alpha), padding = 'same', kernel_initializer = 'he_normal')(conv9)
