...
loss_divisor = float(input_size[0] * input_size[1])
...
def custom_loss(y_true, y_pred):
    ...
    return (tf.reduce_sum(tf.math.squared_difference(y_true_real, y_pred_real), axis=(1,2,3)))
...
#@tf.function
#def merge_dimensions(tensr):
#    # Merges the one-hotted classes back together.
#    # Separate the ground truth and prediction into their classes.
#    tensr_cls0 = tensr[:,:,:,0]
#    tensr_cls1 = tensr[:,:,:,1]
#    tensr_cls2 = tensr[:,:,:,2]
#
#    build_loc = tf.zeros_like(tf.squeeze(tensr), axis=[3])
#
#    merged = tf.logical_and(tf.equal(obj_classes_indexes[:, 0], obj_classes_indexes[0][0]), obj_classes_indexes[:, 1]  > obj_classes_indexes[0][1])
...
def batchmaker(raw_loc, annotated_loc, batchsize, input_size):
    ...
    while True:
        ...
        while batch_end < file_size:
            ...
            # Get the image and target data, which is are saved numpy ndarrays
            # They have already been resized, if necessary.
            x = np.load(img)
            y = np.load(target)
            ...
            y_set = y.reshape((-1, input_size[0], input_size[1], input_size[2]))
