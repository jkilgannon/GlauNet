...
def custom_loss(y_true, y_pred):
    ...
    #
    # We will ignore class 2, which is the "background," since we will be actively
    # ignoring its data (and because it is very VERY hard to keep it from overwhelming
    # the data if we don't ignore it).
    ...
    print(tf.reshape(tf.reduce_mean(loss_0 + loss_1), (-1,1,1,1)).eval())
    sess.close()
