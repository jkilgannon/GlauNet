...
#def dice_coeff_3(y_true, y_pred):
#    mean = 0
#    print("{{{{{{{")
...
#    print("{{{{{{{")
#
#    # Added try/except for first cycle, when y_true is empty JK
#    try:
#        mean = 0
...
def inverted_dice_3(y_true, y_pred):
    return 1 - dice_coeff_3(y_true, y_pred)
...
model.compile(optimizer = Adam(lr = 1e-4), loss = dice_coeff_3, metrics = [inverted_dice_3])
