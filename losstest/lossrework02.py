...
def dice_coefficient_7(y_true, y_pred):
    # Will calculate 2 *|A intersection B| / |A|^2 + |B|^2

    # Will simulate |A intersection B| using A * B.  See https://www.jeremyjordan.me/semantic-segmentation/
    # We won't reduce_sum axis 0 since that is the batch axis.
    AintersectionB = tf.reduce_sum(y_true * y_pred, axis=(1,2,3))

    Asq = tf.reduce_sum(y_true * y_true, axis=(1,2,3))
    Bsq = tf.reduce_sum(y_pred * y_pred, axis=(1,2,3))
    
    return tf.reshape(2 * AintersectionB / Asq + Bsq, (-1,1,1,1))

    #print(str(tf.shape(y_true)))
    #print(str(tf.shape(y_pred)))
    #print(str(y_true.shape))
    #print(str(y_pred.shape))

def soft_dice_loss_7(y_true, y_pred):
    return 1 - dice_coefficient_7(y_true, y_pred)
...
input_unsegmented = '/worksite/fundus/'
input_unsegmented_validate = '/worksite/fundusvalidate/'
input_mask = '/worksite/mask/'
input_mask_validate = '/worksite/maskvalidate/'
...
model.compile(optimizer = Adam(lr = 1e-4), loss = soft_dice_loss_7, metrics = [dice_coefficient_7])
...
batch_gen = batchmaker(input_unsegmented, 
                   input_mask, 
                   batchsize, input_size)

batch_gen_val = batchmaker(input_unsegmented_validate, 
                   input_mask_validate, 
                   batchsize, input_size)
...
model.save(output_path + 'last_weights.h5') 
